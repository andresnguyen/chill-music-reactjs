...
function PlayControl() {
    ...
    const handlePrevClick = () => {
        if (currentIndex === 0) return setCurrentIndex(currentIndex)
    ...
    const handleNextClick = () => {
        ...
        if (currentIndex === songs.length - 1) return setCurrentIndex(currentIndex)
    ...
    return (
        <div className="container-control">
            <div className="play-control">
                <button className={classnames({ 'random-active': isRandom })} onClick={handleRandomClick}>
                    {icon.random}
                </button>
            ...
            <div className="progress-bar">
                <div className="progress-bar__time">{common.formatDate(currentTime)}</div>

                <Slider disabled={false} tooltipVisible={false} value={valueSlider} onChange={handleSliderChange} />

                <div className="progress-bar__time">{common.formatDate(endTime)}</div>
            </div>
